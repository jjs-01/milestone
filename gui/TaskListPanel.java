package gui;// general classes used for formatting
...
public class TaskListPanel extends JPanel {
    private final ArrayList<Tasks> taskList = new ArrayList<>();
    // dynamic stack for deleted tasks
    private final DynamicStack<Tasks> deletedTaskStack = new DynamicStack<>();
    private static final String FONT = "Verdana";
    
    // defining the variables for the main components the user can manipulate
    private final JButton addTaskButton = new JButton("Add new task");
    private final JButton undoButton = new JButton("Undo Delete");
    private final JButton clearButton = new JButton("Clear");
    private final JButton saveButton = new JButton("Save tasks");
    private final JSlider gridSizeSlider = new JSlider(10, 20, 10);
    
    // defining the gridLayout for the table
    private final TaskTablePanel taskTable = new TaskTablePanel();
    
    // defining the arraylists which hold each item in the table
    private final ArrayList<JTextField> taskNames = new ArrayList<>();
    private final ArrayList<JTextField> dueDates = new ArrayList<>();
    private final ArrayList<JComboBox<String>> statusSelection = new ArrayList<>();
    private final ArrayList<JComboBox<String>> prioritySelection = new ArrayList<>();
    
    // this is the name of the text for the file that the tasks are saved to
    private static final String TEXTFILENAME = "gui/taskList.txt";
    ...
    public TaskListPanel() {
        ...
        task.setFont(new Font(FONT, Font.PLAIN, 24));
        ...
        flowContainer.add(buttonPanel);

        JPanel taskListSliderContainer = new JPanel();
        taskListSliderContainer.setLayout(new BoxLayout(taskListSliderContainer,
                BoxLayout.Y_AXIS));

        taskListSliderContainer.add(flowContainer);
        taskListSliderContainer.add(gridSizeSlider);
        
        // the container is then added to this panel.
        add(taskListSliderContainer);
    }
     ...
     */
    private void setComboBoxColour(JComboBox<String> comboBox, int index, int type) {
    ...
    private void addRowToTable() {
        ...
        JComboBox<String> statusBox = new JComboBox<>( statusNames );
        JComboBox<String> priorityBox = new JComboBox<>( priorityNames );
        ...
        taskName.setFont(new Font(FONT, Font.PLAIN, 16));
        ...
        date.setFont(new Font(FONT, Font.PLAIN, 20));
    ...
    class ComboBoxStatusListener implements ItemListener {
        ...
        public void itemStateChanged( ItemEvent  e ) {
            for (int counter = 0; counter < statusSelection.size(); counter++) {
                ...
                JComboBox<String> currentBox = statusSelection.get(counter);
    ...
    class ComboBoxPriorityListener implements ItemListener {
        ...
        @Override 
        public void itemStateChanged( ItemEvent e ) {
            for (JComboBox<String> currentBox : prioritySelection) {
                if (e.getSource() == currentBox) {
